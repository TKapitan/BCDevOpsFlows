...
function Get-BCCTrustedNuGetFeeds {
    ...
    if ($includeMicrosoftNuGetFeeds) {
        ...
        $feedConfig = New-NuGetFeedConfig -name "MSApps2" -url "https://dynamicssmb2.pkgs.visualstudio.com/DynamicsBCPublicFeeds/_packaging/MSApps/nuget/v3/index.json"
        $requiredTrustedNuGetFeeds += @($feedConfig)
        if (-not $skipSymbolsFeeds) {
            $feedConfig = New-NuGetFeedConfig -name "MSSymbols2" -url "https://dynamicssmb2.pkgs.visualstudio.com/DynamicsBCPublicFeeds/_packaging/MSSymbols/nuget/v3/index.json"
            $requiredTrustedNuGetFeeds += @($feedConfig)
