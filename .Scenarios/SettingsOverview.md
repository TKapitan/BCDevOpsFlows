...
| <a id="runNumberOffset"></a>runNumberOffset | when using **VersioningStrategy** 0, the CI/CD workflow uses the Azure DevOps BUILD_NUMBER as the build part of the version number as described under VersioningStrategy. The BUILD_NUMBER is ever increasing and if you want to reset it, when increasing the Major or Minor parts of the version number, you can specify a negative number as runNumberOffset. You can also provide a positive number to get a starting offset. Read about BUILD_NUMBER [here](https://learn.microsoft.com/en-us/azure/devops/pipelines/build/variables?view=azure-devops&tabs=yaml) | 0 |
